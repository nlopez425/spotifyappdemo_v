...
export async function GET(request: NextRequest) {
  ...
  let tokenData: { access_token: string } = { access_token: "" };
  ...
  if(code === null){
    ...
    try{
      if(!NEXT_PUBLIC_SPOTIFY_CLIENT_ID || !SPOTIFY_CLIENT_SECRET || !NEXT_PUBLIC_NEXTAUTH_URL){
      ...
      if(!fetchToken.ok) throw new ApiError("Failed to fetch token", fetchToken.status);
      tokenData = await fetchToken.json();

    }catch(error){
      if(error instanceof ApiError){
        return NextResponse.redirect(`${NEXT_PUBLIC_NEXTAUTH_URL}/auth/error?error=${encodeURIComponent(error.message)}`);
      }else{
        return NextResponse.redirect(`${NEXT_PUBLIC_NEXTAUTH_URL}/auth/error?error=${encodeURIComponent('An unexpected error occurred')}`);
      }
    }finally{
      // Set token in cookie
